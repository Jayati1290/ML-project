...
The downloaded binary packages are in
	/var/folders/88/lmtsc9_j4zdgpz_mrhyfmkdc0000gn/T//RtmpEbwr82/downloaded_packages
> library(swirl)
...
What shall I call you? Jayati

| Thanks, Jayati. Let's cover a couple of quick housekeeping items before we
...
> type ?c
Error in `?`(type, c) : 
  no documentation of type ‘type’ and topic ‘c’ (or error in processing help)
> ?c
...
| Enter z * 2 + 100, without assigning the result to a variable. The `*` symbol is used for
| multiplication and shares a key with the number 8 on many keyboards.
